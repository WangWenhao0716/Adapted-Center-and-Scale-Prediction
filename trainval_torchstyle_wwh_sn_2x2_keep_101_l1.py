...
#net.load_state_dict(torch.load('./ckpt_sn_2x2_keep_101_l1/CSPNet-149.pth.tea'))


#load pretrained model
#net.load_state_dict(torch.load('./pretrained/ResNet50v2+SN(8,32)-77.57.pth'))
...
def train():
    ...
    if not os.path.exists('./ckpt_sn_2x2_keep_101_l1'):
        os.mkdir('./ckpt_sn_2x2_keep_101_l1')
    if not os.path.exists('./mr_sn_2x2_keep_101_l1'):
        os.mkdir('./mr_sn_2x2_keep_101_l1')
    if not os.path.exists('./loss_sn_2x2_keep_101_l1'):
        os.mkdir('./loss_sn_2x2_keep_101_l1')
    ...
    for epoch in range(149,200):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            t4 = time.time()
            print('\r[Epoch %d/200, Batch %d/%d]$ <Total loss: %.6f> cls: %.6f, reg: %.6f, off: %.6f, Time: %.3f sec        ' %
        ...
        name = "./loss_sn_2x2_keep_101_l1/loss_" + str(epoch) + ".npy"
        ...
        if config.val and epoch + 1 > 10 and (epoch + 1) % config.val_frequency == 0:
            cur_mr = val(vallog)#Use which one to val? .pth or .tea?
            mr_list.append(cur_mr)
            mr_out = np.array(mr_list)
            name = "./mr_sn_2x2_keep_101_l1/mr_" + str(epoch) + ".npy"
            np.save(name,mr_out)
        ...
        filename = './ckpt_sn_2x2_keep_101_l1/%s-%d.pth' % (net.module.__class__.__name__,epoch+1)
